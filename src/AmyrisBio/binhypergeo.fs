...
module bhg =
    ...
    let mergePrimesBase (pList1:(int*int) list) (pList2:(int*int) list) op = 
        let rec merge l1 l2 res =
            match l1,l2 with
                ...
                | (p1,c1)::tl1,(p2,c2)::tl2 when p1 < p2 -> merge tl1 l2 ((p1,c1)::res)
                | (p1,c1)::tl1,(p2,c2)::tl2  -> merge l1 tl2 ((p2,op 0 c2)::res)
        merge pList1 pList2 []
    ...
    let expandPrimes p = 
        let top,bot = p |> List.partition (fun (p,exp) -> exp >= 0)
        ...
        let rec combine a b (t:float) =
            match a,b with
                ...
                |(hd::tl),[] -> combine tl [] (t*float(hd))
                | (h1::t1),(h2::t2) when t > 0.0 -> combine a t2 (t/float(h2))
                | (h1::t1),(h2::t2) when t <= 0.0 -> combine t1 b (t*float(h1))
    ...
    let rec fact k r = if k <= 1 then r else fact (k-1) (r*float(k))
